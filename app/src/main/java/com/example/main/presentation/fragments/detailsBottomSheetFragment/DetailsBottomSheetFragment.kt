...
class DetailsBottomSheetFragment :
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        weatherInfo = arguments?.getSerializable(WEATHER_INFO_KEY) as? WeatherInfo
            ?: throw IllegalArgumentException("WeatherInfo object must be provided")
    ...
    private fun initViews(weatherInfo: WeatherInfo) {
        ...
        with(binding) {
            ...
            // Background by type of the weather
            when (weatherInfo.weatherDescription) {
                "Clear" -> root.setBackgroundResource(R.drawable.sunny_bg)
                "Clouds" -> root.setBackgroundResource(R.drawable.cloudy_bg)
                "Haze" -> root.setBackgroundResource(R.drawable.haze_bg)
                "Snow" -> root.setBackgroundResource(R.drawable.snow_bg)
                else -> root.setBackgroundResource(R.drawable.rainy_bg)
            }

            infoRecyclerView.adapter = WeatherParametersAdapter(parameters)
            forecastRecyclerView.adapter = WeatherForecastAdapter(emptyList())
    ...
    private fun getWeatherParameters(weatherInfo: WeatherInfo): ArrayList<WeatherParameter> =
        arrayListOf(
            WeatherParameter(getString(R.string.city), weatherInfo.cityName),
            WeatherParameter(getString(R.string.weather), weatherInfo.weatherDescription),
            WeatherParameter(getString(R.string.temperature),
                getString(R.string.temperature_in_celsius, weatherInfo.temperatureInCelsius)),
            WeatherParameter(getString(R.string.humidity),
                getString(R.string.humidity_as_percentage, weatherInfo.humidityAsPercentage)),
            WeatherParameter(getString(R.string.pressure),
                getString(R.string.pressure_in_millimeters_of_mercury,
                    weatherInfo.pressureInMillimetersOfMercury)),
            WeatherParameter(getString(R.string.wind_speed),
                getString(R.string.wind_speed_in_meters_per_second,
